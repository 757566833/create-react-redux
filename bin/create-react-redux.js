            ...
            if (json.hasOwnProperty(key)) {
                ...
                for (const componentsKey in components) {
                    if (components.hasOwnProperty(componentsKey)) {
                        ...
                        //写入入口文件
                        fs.writeFileSync(`./src/entry/${key}.jsx`, entrystr.replace(/{{key}}/g, key).replace(/{{className}}/g, `${key.slice(0, 1).toUpperCase()}${key.slice(1)}`).replace(/{{imports}}/g, imports).replace(/{{routers}}/g, routers))
                    }
                }
            }
        }
